...
A key concept of sourcecode-editor are processors. These are modules that can be added to hande new type of URL.

Built-in processors are stored in the [sourcecode-editor-server/processors-system](../../sourcecode-editor-server/processors-system/) of thise repository.

However, users can implement their own processors wihout changing the application.

Do do so you need to create a Javascript file in the `/opt/app/sourcecode-editor/processors-user` of the runtime container
...
It is recommenede to have a look at the existing [System Processors](../../sourcecode-editor-server/processors-system/).
